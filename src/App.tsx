...
function AIChat({devices,agents,systemPrompt,placeholder,initMsg}){
  const [chat,setChat]=useState([{role:"ai",text:initMsg||"Hi! I'm Sentinel AI. Ask me anything about your property."}]);
  ...
  const sys=systemPrompt||("You are Sentinel AI. Devices: "+(devices||[]).map(d=>d.name+": "+d.status).join(", ")+". Agents: "+(agents||[]).map(a=>a.name+" ("+a.status+")").join(", ")+". Threat: 12/100. Reply in 1-3 sentences.");
...
function EnergyHub({devices,agents}){
  ...
  const energySys="You are Sentinel PEMS AI. Persona: "+persona+". Load: "+totalLoad.toFixed(2)+"kW. TOU: $"+touRate.toFixed(2)+"/kWh ("+touLabel+"). Battery: "+battery+"% — security autonomy "+secAutonomy+"h. Mode: "+pemsMode+". Priority: Safety > Security > Energy. Never shed Security Mesh unless UPS < 15%. Reply 2-3 sentences with specific kWh/cost savings.";
  ...
  return(
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:960,margin:"0 auto",padding:"0 0 48px",position:"relative",minHeight:700,background:T.bg}}>
      ...
      {tab==="Dashboard"&&(<div>
        ...
        <div style={CS}><p style={{margin:"0 0 12px",fontSize:13,fontWeight:500,color:T.txt}}>Sentinel AI</p><AIChat devices={devices} agents={agents}/></div>
      </div>)}
